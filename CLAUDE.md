...
- 指静脈 Unit(`station/build_vein_unit.py`、`site/vein-unit/`、REV vu1〜): 指静脈モジュールだけをタカチ SW-75 に入れ、端面の Grove 1 口(5V → 中継基板の LDO で 3.3V)で PortABC の PORT.C につなぐ案。机に並べて使う想定。box ヘルパー・指静脈のイメージ形状・プレビューのページ出力は `station/shapes.py`(PF と共有)。
